  ...
  let btnSync = document.createElement('i')
    ...
    navigator.geolocation.getCurrentPosition(GeoPposition => {
      ...
      const marker = L.marker([GeoPposition.coords.latitude, GeoPposition.coords.longitude]).addTo(map)
        ...
        let popup = marker.bindPopup("<i onpointerdown='navigator.share({title: this.data.title, url: this.data.url})' class='fa fa-share' data-title='' data-text='' data-url='' ></i><br />Latitude: " + p0.lat + "<br />Longitude: " + p0.lng + "").openPopup();
